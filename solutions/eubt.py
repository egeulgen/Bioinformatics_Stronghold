...
def Newick_format(tree_in):
    ...
    if len(tree.nodes) == 1:
        ...
        for candidate_node in tree.nodes:
            ...
            adjacent_nodes = [node for edge in adjacent_edges for node in edge.nodes if
                              node in edge.nodes and node is not candidate_node]
...
if __name__ == "__main__":
    ...
    Return: A list containing all unrooted binary trees whose leaves are these n taxa. Trees should be given in 
    Newick format, with one tree on each line; the order of the trees is unimportant.
    '''
    input_lines = sys.stdin.read().splitlines()
    leaves = input_lines[0].split()
